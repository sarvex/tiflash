...
def main():
    ...
    files_to_check = (
        run_cmd('git diff HEAD --name-only')
        if args.diff_from == 'HEAD'
        else run_cmd(f'git diff {args.diff_from} --name-only')
    )
    ...
    for f in files_to_check:
        ...
        if not path.exists(file_path):
            continue
        if ' ' in file_path:
            print(f'file {file_path} can not be formatted')
            continue
    ...
    if args.dump_diff_files_to:
        ...
        print(f'dump {len(da)} modified files info to {args.dump_diff_files_to}')

    if files_to_format:
        print('Files to format:\n  {}'.format('\n  '.join(files_to_format)))
        for file in files_to_format:
            cmd = f'clang-format -i {file}'
